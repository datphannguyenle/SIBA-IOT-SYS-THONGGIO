...
(function (window) {
  "use strict";
  var serial = 0;
  var L = {RUNNING:"Đang chạy",STOPPED:"Đã dừng",ONLINE:"Trực tuyến",OFFLINE:"Ngoại tuyến",CURRENT:"Hiện tại",STALE:"Dữ liệu cũ",UNKNOWN:"Chưa rõ",NORMAL:"Bình thường",ACTIVE:"Đang bật",WARNING:"Cần chú ý",MAJOR:"Nghiêm trọng",NONE:"Không có",NOT_CONFIGURED:"Chưa cấu hình",MANUAL:"Thủ công",AUTO:"Tự động",STEP:"Theo cấp",VFD:"VFD",ENABLED:"Bật",DISABLED:"Tắt",ACTUAL_TEMPERATURE:"Nhiệt độ thực",PERCEIVED_TEMPERATURE:"Nhiệt độ cảm nhận"};
  ...
  function nav(state,b){return 'data-nav="'+state+'"'+(b&&b.id?' data-barn-id="'+esc(b.id)+'"':"");}
  ...
  function header(vm,p){var b=barn(vm,p),st=p.state||"default",scope=[(vm.scope||{}).farm,(vm.scope||{}).area].filter(Boolean).map(esc).join(" · "),badge=demo(vm)?'<b class="vm-badge">'+esc(vm.badgeLabel||"DEMO")+'</b>':"";if(st==="default")return '<header class="vm-header vm-header--home"><div><p class="vm-eyebrow">SIBA · THÔNG GIÓ</p><h1>Tổng quan toàn trại</h1><p class="vm-muted">Giám sát chỉ xem'+(demo(vm)?' · dữ liệu minh họa':"")+'</p></div><div class="vm-header__meta">'+(scope?'<span>'+scope+'</span>':"")+badge+'</div></header>';return '<header class="vm-header"><div class="vm-header__line"><button type="button" class="vm-back" '+nav("default",b)+'>← Tổng quan</button><div><p class="vm-eyebrow">SIBA · THÔNG GIÓ</p><h1>'+esc((b||{}).label||"Nhà chưa chọn")+'</h1></div><span class="vm-header__scope">'+scope+'</span>'+badge+'</div><nav class="vm-tabs">'+[["vent_detail","Giám sát"],["vent_history","Lịch sử"],["vent_alarms","Cảnh báo"],["vent_settings","Cài đặt"]].map(function(x){return '<button type="button" '+nav(x[0],b)+' class="'+(st===x[0]?"is-active":"")+'">'+x[1]+'</button>';}).join("")+'</nav></header>';}
  function card(n,v,note,tone){return '<article class="vm-card '+(tone?'vm-card--'+tone:"")+'"><p>'+esc(n)+'</p><b>'+v+'</b><small>'+esc(note||"")+'</small></article>';}
  function overview(vm){var q=vm.summary||{};return '<section class="vm-overview"><div class="vm-kpis">'+card("Số nhà",esc(q.barns==null?"--":q.barns),"Toàn trại")+card("Trực tuyến",esc(q.online==null?"--":q.online),"Theo nguồn dữ liệu","ok")+card("Cần chú ý",esc(q.attention==null?"--":q.attention),"Theo nguồn dữ liệu","warn")+card("Cảnh báo đang mở",esc(q.activeAlarms==null?"--":q.activeAlarms),"Theo nguồn dữ liệu","danger")+'</div><section class="vm-panel"><div class="vm-panel__head"><div><h2>Chọn nhà</h2><p>Chọn nhà để vào ngữ cảnh giám sát.</p></div><label class="vm-search">Tìm nhà <input type="search" data-filter="barn"></label></div><div class="vm-barn-grid" data-filter-list="barn">'+(vm.barns||[]).map(function(b){var f=[b.label,b.connectivity,b.freshness,b.mode,b.alarm].join(" ").toLowerCase();return '<button type="button" class="vm-barn" '+nav("vent_detail",b)+' data-filter-text="'+esc(f)+'"><b>'+esc(b.label)+(b.identity==="SYNTHETIC"?' <small>MINH HỌA</small>':"")+'</b><span class="'+scls(b.connectivity)+'">'+esc(txt(b.connectivity))+'</span><span>Dữ liệu: '+esc(txt(b.freshness))+'</span><span>Chế độ: '+esc(txt(b.mode))+' · Cấp: '+esc(b.stage==null?"--":b.stage)+'</span></button>';}).join("")+'</div><p class="vm-empty" hidden>Không có nhà phù hợp.</p>'+((vm.barns||[]).length?"":'<p class="vm-notice">'+(demo(vm)?"Fixture demo chưa khai báo nhà nào.":"Chưa có nhà nào: gán datasource nhiều entity (alias theo loại thiết bị) và khai keyMap cho widget này.")+'</p>')+'</section></section>';}
  ...
  function history(vm,p){if(!pilot(vm,p))return notice(vm,p,"lịch sử môi trường");var r=vm.history||[];if(!r.length)return '<section class="vm-panel vm-notice"><h2>Lịch sử môi trường</h2><p>Chưa có mẫu lịch sử.</p></section>';function path(k,d){var down=false;return r.map(function(x,i){if(typeof x[k]!=="number"){down=false;return "";}var y=245-(x[k]-d[0])/(d[1]-d[0]||1)*175,c=down?"L":"M";down=true;return c+(55+i*620/Math.max(1,r.length-1)).toFixed(1)+" "+y.toFixed(1);}).join("");}function dom(keys){var a=[];r.forEach(function(x){keys.forEach(function(k){if(typeof x[k]==="number")a.push(x[k]);});});var n=a.length?Math.min.apply(null,a):0,x=a.length?Math.max.apply(null,a):1,q=x-n||1;return [Math.floor((n-q*.12)*2)/2,Math.ceil((x+q*.12)*2)/2];}var t=dom(["indoorTemperatureAvg","outdoorTemperature","perceivedTemperature"]),h=dom(["relativeHumidity"]);function rows(keys){return r.slice().reverse().map(function(x){return '<tr><td>'+esc(x.ts?new Date(x.ts).toLocaleString("vi-VN"):"--")+'</td>'+keys.map(function(k){return '<td>'+esc(typeof x[k[0]]==="number"?x[k[0]]+" "+k[1]:"--")+'</td>';}).join("")+'<td class="'+scls(x.quality)+'">'+esc(txt(x.quality))+'</td></tr>';}).join("");}var env=[["indoorTemperatureAvg","°C"],["outdoorTemperature","°C"],["perceivedTemperature","°C"],["relativeHumidity","%RH"]],air=[["airSpeed","m/s"],["airFlow","m3/h"],["waterConsumptionTotal","L"]];return '<section class="vm-history"><section class="vm-panel"><h2>Lịch sử môi trường</h2><p class="vm-muted">Nhiệt độ dùng trục °C trái; độ ẩm dùng trục %RH phải. Điểm thiếu tạo khoảng trống.</p><p class="vm-legend"><i class="inside"></i>Trong nhà <i class="outside"></i>Ngoài trời <i class="perceived"></i>Cảm nhận <i class="humidity"></i>Độ ẩm</p><div class="vm-chart-wrap"><svg class="vm-chart" viewBox="0 0 720 280"><path class="grid" d="M55 70H675M55 128H675M55 187H675M55 245H675"/><path class="inside" d="'+path("indoorTemperatureAvg",t)+'"/><path class="outside" d="'+path("outdoorTemperature",t)+'"/><path class="perceived" d="'+path("perceivedTemperature",t)+'"/><path class="humidity" d="'+path("relativeHumidity",h)+'"/><text x="4" y="44">'+esc(t[1])+ '°C</text><text x="678" y="44">'+esc(h[1])+'%</text><text x="55" y="266">'+esc(r[0].ts?new Date(r[0].ts).toLocaleTimeString("vi-VN",{hour:"2-digit",minute:"2-digit"}):"--")+'</text><text x="620" y="266">'+esc(r[r.length-1].ts?new Date(r[r.length-1].ts).toLocaleTimeString("vi-VN",{hour:"2-digit",minute:"2-digit"}):"--")+'</text></svg></div></section><section class="vm-panel"><h2>Bảng môi trường</h2><div class="vm-table-wrap"><table><thead><tr><th>Thời gian</th><th>Trong nhà</th><th>Ngoài trời</th><th>Cảm nhận</th><th>Độ ẩm</th><th>Chất lượng</th></tr></thead><tbody>'+rows(env)+'</tbody></table></div></section><section class="vm-panel"><h2>Gió & nước</h2><p class="vm-muted">Nước là bộ đếm tổng tích lũy; không suy ra lượng theo ngày/lứa.</p><div class="vm-table-wrap"><table><thead><tr><th>Thời gian</th><th>Tốc độ gió</th><th>Lưu lượng gió</th><th>Nước tiêu thụ (tổng)</th><th>Chất lượng</th></tr></thead><tbody>'+rows(air)+'</tbody></table></div></section></section>';}
  function alarms(vm){var unloaded=vm.alarmsSource&&vm.alarmsSource.loaded===false,a=unloaded?[]:(vm.alarms||[]),on=a.filter(function(x){return x.status==="ACTIVE";});return '<section class="vm-alarms"><section class="vm-panel"><h2>Cảnh báo</h2><p class="vm-muted">Phạm vi '+(demo(vm)?'toàn trại trong dữ liệu minh họa':'chưa xác định từ nguồn hiện tại')+'. Chỉ xem.</p>'+(unloaded?'<p class="vm-notice">-- chưa nạp cảnh báo</p>':'<div class="vm-table-wrap"><table><thead><tr><th>Mức</th><th>Nội dung</th><th>Nguồn</th><th>Đối tượng</th><th>Thời gian</th><th>Trạng thái</th></tr></thead><tbody>'+a.map(function(x){return '<tr><td class="'+scls(x.severity)+'">'+esc(txt(x.severity))+'</td><td>'+esc(x.type||"--")+'<br><small>'+esc(x.message||"")+'</small></td><td>'+esc(x.source||"UNKNOWN")+'</td><td>'+esc(x.originator||"--")+'</td><td>'+esc(x.created?new Date(x.created).toLocaleString("vi-VN"):"--")+'</td><td>'+esc(txt(x.status))+'</td></tr>';}).join("")+'</tbody></table></div>')+'</section><aside class="vm-panel vm-alarm-summary"><h2>Tóm tắt</h2><b>'+esc(unloaded?"--":on.length)+'</b><span>'+(unloaded?'chưa nạp':'đang hoạt động')+'</span></aside></section>';}
  ...
  function render(c, vm, component, navigate, p) {
    ...
    function click(event) {
      ...
      if (typeof navigate === 'function') navigate(target.getAttribute('data-nav'), {barnId: target.getAttribute('data-barn-id') || p.barnId});
    }
